...
void initGrid()
{
    ...
    for (Position *playerPosition = playerPositions; playerPosition != NULL; playerPosition = playerPosition->next) {
...
void moveTo(Position *playerPosition, int x, int y)
{
    ...
    for (Position *ptr = playerPositions; ptr != NULL; ptr = ptr->next) {
        if (ptr->x == x && ptr->y == y) {
            return;
        }
    }
